...
namespace SS2.SReportTool.Core
{
    class ImportCore
    {
        ...
        void SaveToDB(DataTable dt)
        {
            ...
            var conn = RSetting.DataSource.ConnectionString;

            //var dataHelper = new TableHelper.DataHelper(conn);
            var columns = new List<ReportColumn>();

            //匹配可导入的列
            foreach (var col in RSetting.XCols)
            {
                foreach (DataColumn dbCol in dt.Columns)
                {
                    ...
                    if (existColumn)
                    {
                        if (!columns.Any(i => i.Field == col.Field))
                        {
                            columns.Add(col);
                        }
                    }
            ...
            if (columns.Count > 0)
            {
                RSetting.DataSource.DataAdapter.InsertReportData(RSetting, columns, dt);
            }
            else
            {
