...
namespace SS2.SReportTool.Core
{
    ...
    class ColunmFormater
    {
        ...
        /// <returns></returns>
        public static string FormatCell(DataRow row, string fieldName, Type dataType, string format)
        {
            if (dataType==null)
            {
                dataType = (row.Table.Columns[fieldName]).DataType;
            }
            //当前值
            ...
            if (flines.Length > 1)
            {
                ...
                for (int i = 0; i < flines.Length; i += 2)
                {
                    ...
                    {
                        return FormatValue(row, fieldName, formatstring);
                    }
                }
                if (!string.IsNullOrEmpty(defFormat)) { return FormatValue(row, fieldName, defFormat); }
                return v.ToString().Trim();
            }
            else
            {
                return FormatValue(row, fieldName, format);
            }
        }
        ...
        /// <param name="dataType"></param>
        /// <returns></returns>
        static bool EqualsSetting(string caseValue, object cellValue, Type dataType)
        {
